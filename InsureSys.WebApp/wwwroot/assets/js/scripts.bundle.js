    ...
    $.fn[pluginName] = function (options) {
        ...
        var Plugin = {
            ...
            sortCallback: function (data, sort, column) {
                ...
                return $(data).sort(function (a, b) {
                    ...
                    if (typeof aField != 'string') {
                        aField = aField.toString();
                    }
                    if (typeof bField != 'string') {
                        bField = bField.toString();
                    }
                    var aFieldNumeric = aField.replace(/[$ ,%]/g, '');
                    var bFieldNumeric = bField.replace(/[$ ,%]/g, '');
                    if (
                        ((aField.indexOf('AM') > -1 || aField.indexOf('PM') > -1) && aField.indexOf('-') > -1)
                        && ((bField.indexOf('AM') > -1 || bField.indexOf('PM') > -1) && bField.indexOf('-') > -1)
                    ) {

                        var aFieldIntervalAMPrefix = aField.split('-')[0];
                        var aFieldIntervalPMPrefix = aField.split('-')[0];

                        if (aFieldIntervalAMPrefix.indexOf('AM') > -1) {
                            aFieldIntervalAMPrefix = aFieldIntervalAMPrefix.replace('AM', '');
                            aField = parseInt(aFieldIntervalAMPrefix);

                        }
                        else if (aFieldIntervalPMPrefix.indexOf('PM') > -1) {
                            aFieldIntervalPMPrefix = aFieldIntervalPMPrefix.replace('PM', '');
                            aFieldIntervalPMPrefix = parseInt(aFieldIntervalPMPrefix);
                            if (aFieldIntervalPMPrefix !== 12) {
                                aField = 12 + aFieldIntervalPMPrefix;
                            }

                        }

                        var bFieldIntervalAMPrefix = bField.split('-')[0];
                        var bFieldIntervalPMPrefix = bField.split('-')[0];

                        if (bFieldIntervalAMPrefix.indexOf('AM') > -1) {
                            bFieldIntervalAMPrefix = bFieldIntervalAMPrefix.replace('AM', '');
                            bField = parseInt(bFieldIntervalAMPrefix);

                        }
                        else if (bFieldIntervalPMPrefix.indexOf('PM') > -1) {
                            bFieldIntervalPMPrefix = bFieldIntervalPMPrefix.replace('PM', '');
                            bFieldIntervalPMPrefix = parseInt(bFieldIntervalPMPrefix);
                            if (bFieldIntervalPMPrefix !== 12) {
                                bField = 12 + bFieldIntervalPMPrefix;
                            }
                        }
                        type = 'number';
                    }
                    else if ($.isNumeric(aFieldNumeric) && $.isNumeric(bFieldNumeric)) {
                        type = 'number';
                    }
                    ...
                    switch (type) {
                        ...
                        case 'number':
                            if (isNaN(parseFloat(aFieldNumeric)) && aFieldNumeric != null) {
                                aFieldNumeric = Number(aFieldNumeric.replace(/[^0-9\.-]+/g, ''));
                            }
                            if (isNaN(parseFloat(bFieldNumeric)) && bFieldNumeric != null) {
                                bFieldNumeric = Number(bFieldNumeric.replace(/[^0-9\.-]+/g, ''));
                            }
                            aField = parseFloat(aFieldNumeric);
                            bField = parseFloat(bFieldNumeric);
                            if (sort === 'asc') {
            ...
            rowEvenOdd: function () {
                ...
                if ($(datatable.tableBody).find('.' + pfx + 'datatable-row > td[aria-label~="#Total"]').length > 0) {
                    var $finaltr = $(datatable.tableBody).find('.' + pfx + 'datatable-row > td[aria-label~="#Total"]').closest('tr');
                    if ($finaltr != undefined && $finaltr.length === 1) {
                        $finaltr.css('background-color', '#ddd');
                        $finaltr.css('font-weight', 'bold');
                    }
                }
            },
...
KTUtil.ready(function () {
    ...
    // Init Quick Cart Panel
    KTLayoutQuickReciept.init('kt_quick_reciept');
});
...
var KTLayoutQuickReciept = function () {
    // Private properties
    var _element;
    var _offcanvasObject;
    ...
    var _init = function () {
        _offcanvasObject = new KTOffcanvas(_element, {
            ...
            baseClass: 'offcanvas',
            placement: 'right',
            closeBy: 'kt_quick_reciept_close',
            toggleBy: 'kt_quick_reciept_toggle'
        });
...
// Webpack support
if (typeof module !== 'undefined') {
    module.exports = KTLayoutQuickReciept;
}
